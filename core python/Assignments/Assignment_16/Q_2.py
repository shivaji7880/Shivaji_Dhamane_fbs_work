class Product:
    
    discountt=15
    def __init__(self,pid,pname,price,quantity):
        self.pid=pid
        self.pname=pname
        self.price=price
        self.quantiy=quantity
        
    def getPrice(self):
        p=self.price
        #print(f'Price:{p}')
        return p
        
    def discount(self,p):
        p_after_dis=p-(Product.discountt*p/100)
        print(f'P_after_dis:{p_after_dis}',end='  ')
    
    def showProduct(self):
        print(f'Pid:{self.pid}\tPname:{self.pname}\tPrice:{self.price}\tQuantity:{self.quantiy}')
        
    def __del__(self):
        print('Object is destroy...') 
    
p1=Product(732,'Watch',1299,3)
p2=Product(645,'Laptop',86999,2)
p3=Product(389,'NoteBook',79,4)
p1.discount(p1.getPrice())
p1.showProduct()
p2.discount(p2.getPrice())
p2.showProduct()
p3.discount(p3.getPrice())
p3.showProduct()
print()
print(f'Price:{p1.getPrice()}')
